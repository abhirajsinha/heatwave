...
- **Task packet (R-148, v5-retrieval):** when a `00-task-packet.md` is attached, start from it — ranked relevant files, their tests, import neighbours, matching failure-memory — instead of re-exploring the tree. Advisory starting point, **never a gate**: verify before relying, and an out-of-packet read is expected and legal, recorded via R-49.
