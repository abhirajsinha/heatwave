...
| Atlassian MCP | Intake (Jira mode) | Jira key/URL in the task (R-127) | low | auto-when-present |
...
| [Atlassian Remote MCP](https://www.atlassian.com/platform/remote-mcp-server) (Atlassian) | Jira-mode intake channel (v4.3, R-127): the driver fetches the ticket read-only (`getJiraIssue`) into a Requirement Brief; absent → `NOT AVAILABLE` + paste fallback, never a silent text run. Read-only in V1 is a **behavioral guarantee** — the R-127 mandate that Heatwave invokes only read tools (`getJiraIssue` and the resource lookup), never a write/transition tool — **not** an OAuth scope restriction: Atlassian's OAuth grant bundles `read:jira-work write:jira-work` together, so the token may carry write capability the protocol never exercises. Grant a read-only-scoped token if your org supports narrowing it. | Claude: `claude mcp add --transport http atlassian https://mcp.atlassian.com/v1/mcp` (OAuth on first use) · Codex: `codex mcp add atlassian -- npx -y mcp-remote https://mcp.atlassian.com/v1/mcp` · Gemini: `gemini mcp add atlassian https://mcp.atlassian.com/v1/mcp` — official endpoint per Atlassian's docs | Proprietary — official, free |
