...
This project uses the **Heatwave AI Development & Verification Protocol**. Full rendered spec: `.heatwave/PROTOCOL.md` (generated). Your working set: `.heatwave/protocol/core.md` + the shard for your role (`.heatwave/protocol/`).

## Single-context rule

Your tool runs one context at a time, so the three AI roles are played by **separate sessions**, never one:

- Each session performs exactly ONE role for a task: PLANNER (`.heatwave/prompts/planner.md`), IMPLEMENTER (`.heatwave/prompts/implementer.md` / `fixer.md`), REVIEWER (`.heatwave/prompts/plan-reviewer.md` / `reviewer.md` / `final-reviewer.md`), or EXPRESS checker (`.heatwave/prompts/express-checker.md`) — a fresh session that did not make the change. One role may span stages: the session acting as REVIEWER for a task SHOULD stay the reviewer for that task's later review passes (R-117 — persistent session, warm context); it must simply never have implemented or planned that task. FINAL_REVIEW still re-runs machine gates from scratch and re-confirms every AC (R-117 safety clause) and reviews only the delta since the last FULL_REVIEW (R-118).
...
- No implementation before a Planning Document passes PLAN_REVIEW with 0 Blockers / 0 Majors — except the EXPRESS tier: one independent machine-gated check gates APPROVED (R-104).
