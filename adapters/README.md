...
An adapter is the only tool-specific piece of Heatwave: a small shim that puts the binding rules where a given agent reads its standing instructions. Everything else — the `protocol/` shards and generated PROTOCOL.md, prompts, templates, the run directory — is identical for every agent.
...
2. Create `adapters/<tool>/` with a file that says, in order:
   - This project runs under Heatwave; read `.heatwave/HEATWAVE-AGENT.md` then `.heatwave/protocol/core.md` + your role's shard before production-bound changes (the full rendered spec, `.heatwave/PROTOCOL.md`, is the generated reference).
   - The one-paragraph summary of what binds the agent (copy it from `codex/AGENTS.md` — resume rule, one role per session, plan before code with the EXPRESS and LIGHT carve-outs, evidence, non-stop, ponytail).
