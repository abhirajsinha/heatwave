...
2. Create `adapters/<tool>/` with a file that says, in order:
   ...
   - The one-paragraph summary of what binds the agent (copy it from `codex/AGENTS.md` — resume rule, one role per session, plan before code with the EXPRESS carve-out, evidence, non-stop, ponytail).
