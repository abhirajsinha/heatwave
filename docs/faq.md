...
That is what tiers are for (§0.5). A trivial single edit runs **EXPRESS** (v4): the implementer makes the change and one fresh context runs a machine-gated check — no Planning Document at all. A LIGHT-tier change collapses the Planning Document to four sections and merges the two post-implementation reviews into one pass — the plan is still reviewed before any code is written for every tier except EXPRESS, whose gate is an independent machine-checked verification instead of a plan (R-104). What never goes away: a separate context checks the work, and claims of verification carry evidence. If even LIGHT is too much, the change is probably a spike — label it as such (§0.4) and it is out of protocol scope.
