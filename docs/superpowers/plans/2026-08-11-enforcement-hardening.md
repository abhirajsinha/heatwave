...
task_id: 2026-08-11-enforcement-hardening | artifact_type: planning-document | iteration: 1 | produced_by: PLANNER (claude-fable-5) | timestamp: 2026-08-11
...
During no-edit states (PLANNING, PLAN_REVIEW, FULL_REVIEW, TARGETED_REVIEW, FINAL_REVIEW, EXPRESS_CHECK) the Claude Code `PreToolUse` gate blocks Edit/Write tool calls but not source modification through the Bash tool (`sed -i src/x`, `echo … > src/x`, `python3 -c 'open("src/x","w")…'`). The `heatwave-planner` subagent carries Bash, so the externally-verified bypass is live in every installed project. G1 extends the gate to also match Bash and block *common* shell source-writes — best-effort by design, honestly documented as still not a sandbox. For: every Heatwave-on-Claude-Code user.
...
- FR-4: The existing allowlist governs Bash write targets too: `.heatwave/**`, `CLAUDE.md`/`AGENTS.md`/`GEMINI.md`, `design_doc_path/*.md` (config-read, default `docs/design`), plus any path outside the project root (`/tmp`, `/dev/null`, …) — writes outside the repo are not project source.
...
- FR-6: The ceiling is marked in-code (`ponytail:` comment naming known evasions + pointing at the OS-sandbox track) and the README/FAQ copy says "hardened, still **not a filesystem sandbox**; bypasses land in the audit trail".
...
|---|---|---|---|
| 1 | `>` / `>>` redirection | operator token `>` or `>>` (punctuation split), or fallback prefix `^\d*>{1,2}`; target = next token | target not `allowed_target` |
...
| 8 | `patch` | command-position `patch` | block (writes files in cwd; targets unknowable) |
...
| 16 | interpreter one-liners | command-position `python`/`python3`/`perl`/`ruby`/`node` + a `-c`/`-e` flag token (incl. combined `-pe`/`-ne`); code = the token after the flag | code matches `open\s*\([^)]*['\"](?:[wax]\|r\+)\|writeFile\|appendFile\|createWriteStream\|File\.write\|>` AND code does not contain `.heatwave/` |
...
# evasions: a helper script that writes (printf > /tmp/w.sh && sh /tmp/w.sh);
# a here-doc piped to an interpreter; base64-decoded content; chmod +x && ./writer;
...
| `shlex` ValueError (unbalanced quotes/heredocs) | whitespace-split fallback + prefix redirect scan; err toward allow |
...
8. `git apply --check p.patch` — read-only mode → allow.
9. Quoted targets with spaces (`> "my file"`) — shlex handles → blocked if in-project.
10. `echo x > .heatwave/runs/r/notes` and `tee docs/design/t.md` — allowlisted → allow.
11. Pipelines/chains (`a && sed -i … src/x`, `$(sed -i …)`) — per-segment command position → caught.
12. Heredoc bodies (`cat <<EOF … EOF`) — stripped; literal `>` in body is not a false positive; interpreter-fed heredocs are a named evasion.
13. Multiple runs, only one in NO_EDIT — first NO_EDIT match gates (unchanged semantics).
14. Symlinked target into the project — `os.path.realpath` resolves; symlink tricks beyond that are ceiling.
15. Nested same-named design dir (`vendor/docs/design/x.md`) — still gated (existing anchor, regression-tested).

## Risks

| Risk | Likelihood | Mitigation |
|---|---|---|
| False positive breaks legitimate review shell | Medium (the expensive one) | quote-aware tokenizer; command-position matching; outside-project targets allowed; `--check` guard; AC-F-02's 15-command allow matrix; reviewer hunts more |
| False negative (bypass) | Certain — by design | documented + demonstrated ceiling (AC-F-06); copy says best-effort; audit trail |
...
gate '{"tool_name":"Bash","tool_input":{"command":"sed -i s/a/b/ src/app.py"}}'   # expect exit=2
```
...
- **T1 — `adapters/claude-code/role-gate.sh`:** update header comment (matcher now `Edit|Write|Bash`); factor NO_EDIT-run scan + dd_root into shared code (behavior-identical); add Bash branch per Architecture (tokenizer, denylist table #1–#16, `allowed_target`, block message); add the ceiling `ponytail:` comment verbatim. Satisfies FR-2/3/4/5/6-part.
- **T2 — `install.sh`:** matcher string change + existing-entry migration. Satisfies FR-1.
- **T3 — copy:** `README.md:146`, `docs/faq.md:28` per Copy changes; confirm-and-leave `HEATWAVE.md`, `adapters/README.md`; do NOT touch `protocol/history.md`. Satisfies FR-6-part.
...
**AC-F-03 | IMPLEMENTING (and FIXING) allow everything** | Verification: with `STATE=state: IMPLEMENTING` (then `state: FIXING`), the AC-F-01 `sed -i`, `echo >`, `python3 -c` and `git restore` JSONs each exit **0**.
...
**AC-F-05 | No active run → allow** | Verification: with `.heatwave/runs/` empty, the AC-F-01 `sed -i` and `echo >` JSONs and the AC-F-04 Edit JSON each exit **0**.

**AC-F-06 | Ceiling bypass demonstrated honestly** | Verification: with `STATE=state: FULL_REVIEW`, `{"tool_name":"Bash","tool_input":{"command":"printf 'echo pwned > src/app.py' > /tmp/w.sh && sh /tmp/w.sh"}}` exits **0** (slips through: the redirect target is outside the project; `sh /tmp/w.sh` matches no pattern). Output recorded in the implementation package as the documented ceiling, matching the copy's caveat and the `ponytail:` comment — NOT hidden or "fixed".

**AC-F-07 | Copy honest** | Verification: `grep -n "sandbox\|shell source-writes" README.md docs/faq.md` shows the hardened wording AND the retained "not a filesystem sandbox" caveat in both files; `grep -inE "physically (block|prevent)|completely prevent|cannot bypass" README.md docs/faq.md adapters/` returns nothing.
...
**AC-F-09 | Bypass closure red→green** | Verification: run the AC-F-01 `sed -i`/`echo >`/`python3 -c` JSONs against the **pre-change** `role-gate.sh` (from `git stash`/`git show HEAD:…`) in the same scratch setup → each exits **0** (red: bypass live); against the changed gate → each exits **2** (green). Both outputs pasted.
...
✓ injection — regex/tokenizer over hostile command strings; quoting/heredoc/substitution corners are the bypass surface
