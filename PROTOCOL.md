...
*(v5.0)* **Runtime-evidence rung.** STANDARD, FULL, and the LIGHT combined pass add a `runtime-evidence` rung to the R-110 ladder: the REVIEWER runs `heatwave-report-check.sh runtime-evidence <plan> <acceptance-table>`, which fails when any `runtime: yes` acceptance criterion is signed off by evidence that names no `verification_matrix` runtime class (R-133). A red rung is a machine finding (R-111), default Major. It is the deterministic half of "drive the real product before APPROVED"; the reviewer's product execution (R-133) is the substance the rung checks the completeness of.
...
**R-140.** *(v5.0)* **One PLAN_REVIEW round for non-sensitive STANDARD UI/bugfix runs.** The `plan_iterations` budget is a lookup keyed on tier, change class, and change surface, not a flat 3. It is **1** for a STANDARD run that is `change_class ∈ {ui, bugfix}` **and** touches no R-102 sensitive path (auth, payments/money, user data, schema/migrations, public API) **and** whose `change_surface` (R-122) intersects none of {auth, payments, external-input, new-endpoint, secrets, api-surface}. Every other STANDARD run — any non-UI/non-bugfix feature or refactor, anything sensitive — and every FULL run keep the default **3** (R-13). LIGHT and EXPRESS have no PLAN_REVIEW (R-123, §0.5), so this never applies to them. **Failure semantics:** the single round still runs; if it returns an open Blocker or Major the budget is exhausted at once (1 → one rejection → exhaustion) and the run goes to `ESCALATED` for an OWNER decision — a bad low-risk plan is surfaced to the OWNER, never silently proceeded, never looped. The downstream runtime gate (R-133) is the real backstop the audit shows plan rounds are not; this rule spends fewer plan rounds where the evidence says they rarely paid, not where risk lives. A project MAY restore 3 via the R-13 config override with OWNER approval.
...
run_config:
  ...
  verification_matrix: {}  # v5.0 (R-133): map of evidence class -> concrete project driver, e.g. { web-ui: <browser-driver>, deploy: <deploy-smoke command> }; absent/empty = no runtime class mapped (each required class then NOT AVAILABLE, R-64)
  exploratory_flows: []    # v5.0 (R-134): project-declared real user/operator flows the REVIEWER exercises for the touched surface; absent = none declared
```

The seven runtime evidence **classes** are fixed by the protocol — `web-ui`, `chrome-extension`, `api`, `db-migration`, `mobile`, `deploy`, `real-input` — but no class binds to a named tool here: `verification_matrix` (config, or an adapter) maps each class a project uses to a concrete driver. A class a change requires with no project mapping is `NOT AVAILABLE` (R-64), naming the acceptance criteria it leaves unverified. `verification_matrix` and `exploratory_flows` are behavior-driving from v5.0; absent, the runtime rung and the flow pass are vacuous and the run behaves exactly as pre-v5 for the non-runtime path.
...
|---|---|---|---|
| **Blocker** | Breaks functionality, security, or data integrity. Prevents build, deploy, or safe operation. Includes: undeclared deviation (R-22), asserted verification without evidence (R-65), false tooling claim (R-63), a runtime defect observed on the real product (R-135). | Yes | Only by OWNER waiver |
...
| **Minor** | Suboptimal but correct. Maintainability, non-blocking UX, docs. Includes `paperwork` findings (R-139), which never exceed Minor. | No | Yes |
...
**R-135.** *(v5.0)* **An observed runtime defect is a non-deferrable Blocker.** A failure the REVIEWER observes while driving the real built product (R-133) is a Blocker clearable only by an OWNER waiver (R-9). The REVIEWER MUST NOT downgrade it below Blocker, and it is **outside the R-6 deferral path** — the reviewer-approved deferral R-6 grants for a Major does not reach a defect observed on the real product; only the OWNER waiver (R-9) can clear it. This closes the escape where a runtime defect was caught, deferred, and shipped: a thing that does not work for the user cannot be paperworked or deferred out of the gate.

**R-139.** *(v5.0)* **Paperwork findings are capped below Major.** A finding about acceptance-criterion wording, evidence formatting, tooling-declaration accuracy, or plan-conformance bookkeeping **with no behavioural consequence** carries `Category: paperwork` and MUST NOT exceed Minor — a fixed category, not reviewer mood; it never gates (R-78). Anything with a behavioural consequence is, by definition, not paperwork and keeps its real severity. A tooling-presence fact (is a scanner on PATH) is a script result, not a finding; automatable checks are automated, not filed as prose findings. ~40% of the audit's gating findings were paperwork holding real work hostage — this returns that budget to what breaks for a user.
...
*(v5.0)* The (b) machine-gate re-run and (d) AC re-confirmation carry the runtime engine into FINAL: the `runtime-evidence` and `knowledge-regression` rungs (R-110) are re-run from scratch; every `runtime: yes` criterion is re-confirmed from evidence that drove the real product (R-133), never a read; the touched-surface `exploratory_flows` are re-exercised (R-134); and for a `change_surface ∋ deploy` change the REVIEWER runs `templates/deploy-smoke.sh` against the target and cites it against the deploy AC (R-137). An observed runtime defect at FINAL is a non-deferrable Blocker (R-135). The FINAL report itself opens with the plain-language section and a "For the engineer" split (R-142) and passes `heatwave-report-check.sh plain-language`.
...
*(v5.0)* Three verification-engine duties ride the driver's existing steps, none adding a dispatch, state, or gate: **(1) knowledge supply (R-138)** — before a FULL_REVIEW dispatch, the driver reads `.heatwave/knowledge/`, selects the entries whose `trigger_paths` (string-prefix) intersect the diff, and attaches them to the reviewer dispatch (the same normalization class as the R-131 context brief; R-83 intact). **(2) report checks (R-142/R-133/R-138)** — after each report artifact lands, the driver runs `heatwave-report-check.sh plain-language <report>` on it, and, on review reports carrying an acceptance table or a knowledge-regression duty, `runtime-evidence` and `knowledge-regression`; a red check is recorded and returned to the authoring role as a required fix, never waved through (post-artifact bookkeeping, R-131 precedent). **(3) matrix/flow verdicts** — the driver copies `run_config.verification_matrix` and `exploratory_flows`, and each review's per-runtime-AC and per-flow verdicts, from the plan and review artifacts into the Run Record. These are driver reads and records, never role authority (R-83).
...
### 9.12 Task packet *(v5-retrieval)*

**R-148.** *(v5-retrieval)* **A cached-map-derived, LLM-free, task-scoped retrieval packet — advisory, never a gate.** When `context_packet` is enabled (config; default `never` — opt-in, see below), the driver runs the generator `templates/task-packet.sh` after the repo map (R-146) and before the PLANNING dispatch (STANDARD/FULL) or the IMPLEMENTING dispatch (LIGHT), writing `.heatwave/runs/<id>/00-task-packet.md` and attaching it to every role dispatch (the same normalization class as the R-131 context brief and the R-146 map; R-83 intact — the driver derives, it does not plan). The packet is **paths, counts and line hints only — never a file-body excerpt** (R-131 discipline), and lists, each headed by its quoted derivation command: ranked, budget-capped relevant files (fixed-string `git grep -lF` over tracked files, ranked by distinct-term match count, tie-broken by named-module match then import-degree); their related tests and import neighbours (read from the cached map, not recomputed); an acceptance-criteria pointer (the plan file once it exists — ACs do not exist at packet time); and matching failure-memory entries (`.heatwave/knowledge/` whose `trigger_paths` prefix-intersect the ranked files, the R-138 selection class).

**The packet is a starting point, not a gate.** Reading outside the packet is legal and is recorded via the existing R-49 scope-expansion mechanism; **no correctness gate references packet completeness**, and a role verifies the packet before relying on it (R-131 discipline). A miss is a recorded R-49 expansion, never a hard failure. The generator invokes **no model** and adds **no new dependency, state, tier, or role**. Untrusted-input discipline mirrors the context brief: task terms are passed as **fixed-string arguments** (`git grep -lF -e <term>`), never interpolated into shell syntax; the per-role file cap comes from `context_budgets.<role>` (config), read as an **integer only** with a safe default (12) on an absent or malformed value.

Per-role size: `context_budgets.<role>` caps the ranked-file list; absent/unset → a single default cap (12). This governs packet **size**, never read enforcement (which stays R-49 recording + the R-143/R-144 implementer slicing).

Recording and skips, recorded never silent: the driver records `run_config.context_packet: emitted | skipped-<reason>`. No packet is written when the working directory is **not a git repo** (`skipped-not-a-git-repo`) or when config leaves `context_packet` at its default/`never` (`skipped-config`). **Default is `never` (opt-in):** the packet ships enabled only where a run sets `context_packet: auto`, because its token and correctness value are deferred to a future benchmark and the OWNER roadmap rule is "build each layer only when the benchmark proves it." A killed session resumes on the packet already on disk — immutable (R-89), never re-derived. The packet adds **no role dispatch, no state, no counter**.
